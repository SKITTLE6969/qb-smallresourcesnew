				...
				if CheckWeapon(newWeap) then
					if holstered then
						if WearingHolster then
							...
							local newRot = GetEntityRotation(ped, 5)
							TaskPlayAnimAdvanced(ped, "rcmjosh4", "josh_leadout_cop2", newPos.x, newPos.y, newPos.z, newRot.x, newRot.y, newRot.z, 3.0, 3.0, -1, 50, 0, 0, 0)
							Wait(300)
					...
					elseif newWeap ~= currWeapon and CheckWeapon(currWeapon) then
						if WearingHolster then
							...
							local newPos = GetEntityCoords(ped)
							local newRot = GetEntityRotation(ped, 5)
							TaskPlayAnimAdvanced(ped, "reaction@intimidation@1h", "intro", newPos.x, newPos.y, newPos.z, newRot.x, newRot.y, newRot.z, 8.0, 3.0, -1, 50, 0, 0, 0)
							Wait(1000)
							SetCurrentPedWeapon(ped, newWeap, true)
							currWeapon = newWeap
							Wait(1400)
